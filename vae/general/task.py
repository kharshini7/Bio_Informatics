...
class VAE(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_logs = {f"train_{k}": v for k, v in logs.items()}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 0th batch (batch_idx == 0), self.val_outs is set to batch
        if batch_idx == 0:
            self.val_outs = batch
        return loss
        """
    ...
    def test_step(self, batch, batch_idx):
        ...
        # ensuring that the 0th batch (batch_idx == 0), self.test_outs is set to batch
        if batch_idx == 0:
